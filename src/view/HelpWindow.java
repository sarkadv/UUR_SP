...
public class HelpWindow extends Stage {
	...
	private void createDefaultChildren(TreeItem<HelpItem> root) {
		...
		TreeItem<HelpItem> newFile = new TreeItem<HelpItem>(new HelpItem("Nová mapa", ImageLoader.HINT_NEW, ImageLoader.NEW_LIGHT, "Ctr + N", "Nejdříve se zeptá uživatele, zda si přeje uložit stávající soubor, a poté umožní vytvořit nový."));
		TreeItem<HelpItem> saveFile = new TreeItem<HelpItem>(new HelpItem("Uložit mapu", ImageLoader.HINT_SAVE, ImageLoader.SAVE_LIGHT, "Ctr + S", "Dovolí uživateli uložit mapu ve formě textového souboru."));
		TreeItem<HelpItem> loadFile = new TreeItem<HelpItem>(new HelpItem("Načíst mapu", ImageLoader.HINT_LOAD, ImageLoader.LOAD_LIGHT, "Ctr + L", "Nejdříve se zeptá uživatele, zda si přeje uložit stávající soubor, a poté umožní načíst mapu \nz textového souboru."));
		TreeItem<HelpItem> export = new TreeItem<HelpItem>(new HelpItem("Exportovat jako obrázek", ImageLoader.HINT_EXPORT, ImageLoader.EXPORT_LIGHT, "Ctr + E", "Dovolí uživateli exportovat obrázek mapy a uložit jej ve formátu PNG."));
	
		TreeItem<HelpItem> mapItem = new TreeItem<HelpItem>(new HelpItem("Práce s mapou", ImageLoader.HINT_DEFAULT));
		TreeItem<HelpItem> back = new TreeItem<HelpItem>(new HelpItem("Zpět", ImageLoader.HINT_BACK, ImageLoader.BACK_LIGHT, "Ctr + Z", "Vrátí mapu do stavu před poslední provedenou akcí."));
		TreeItem<HelpItem> mode = new TreeItem<HelpItem>(new HelpItem("Změna barevného režimu", ImageLoader.HINT_MODE, ImageLoader.MODE_LIGHT, "Ctr + M", "Poskytuje uživateli možnosti přepínat ze světlého režimu na tmavý a naopak."));
		...
		TreeItem<HelpItem> eyedropper = new TreeItem<HelpItem>(new HelpItem("Kapátko", ImageLoader.HINT_EYEDROPPER, ImageLoader.EYEDROPPER_LIGHT, "Shift + E", "Nástroj Kapátko dovolí uživateli kliknout na neprázdnou dlaždici a tím ji zvolit \njako vybranou dlaždici místo hledání v menu dlaždic."));
		...
		TreeItem<HelpItem> settings = new TreeItem<HelpItem>(new HelpItem("Nastavení", ImageLoader.HINT_SETTINGS, ImageLoader.SETTINGS_LIGHT, "Ctrl + O", "Zobrazí okno pro nastavení, kde může uživatel změnit velikost mapy, počet viditelných dlaždic,\nbarvu planety a barevný režim."));
